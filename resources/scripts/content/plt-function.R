...
plt_function <- function(
  ...
  if (!is.na(x_lim)) {x_min <- -x_lim; x_max <- x_lim}
  if (!is.na(y_lim)) {y_min <- -y_lim; y_max <- y_lim}
  ...
  for (i in seq_along(func)) {
    if (is.list(func)) {
        ...
        plt_df[temp_rows, "func_label"] <- func[[i]][2]
    } else {
        temp_rows <- (1+(i-1)*nx):(nx + (i-1)*nx)
        plt_df[temp_rows, "x"] <- x
        plt_df[temp_rows, "y"] <- eval(str2expression(func[i]))
        plt_df[temp_rows, "func_label"] <- func[i]
    }
  }

  if (is.null(plt_title)) plt_title <- "Plot"
  if (is.null(ylab)) {
    ylab <- str2expression(paste0("bold(f(",
                                  stringr::str_to_lower(xlab), "))"))
  }

  if (is.null(x_brks)) {x_brks <- waiver()}
  if (is.null(x_minor_brks)) {x_minor_brks <- waiver()}
  if (is.null(x_labs)) {x_labs <- waiver()}
  if (is.null(y_brks)) {y_brks <- waiver()}
  if (is.null(y_minor_brks)) {y_minor_brks <- waiver()}
  if (is.null(y_labs)) {y_labs <- waiver()}

  if (is.na(y_min)) {y_min <- min(min(plt_df$y), -abs(plt_df$y))}
  ...
  if (any(!is.na(xc))) {
    if (nxc > 0 && !is.na(xc)) {
      ...
      plt_df$flag_xc[plt_df$x <= xc[1]] <- paste("1:", xlab,
                                                 expression("\u2264"), xc[1])
      plt_df$flag_xc[plt_df$x >= xc[nxc]] <- paste(nxc + 1, ":", xlab, ">",
                                                   xc[nxc])
      for (j in 1:nxc) {
  ...
  if (facets) {
    ...
    plt <- plt +
      geom_line(mapping = aes(x = x, y = y, color = func_label),
        linewidth = linewidth) +
      facet_wrap(facets = .~ func_label, scales = scales_facet,
    ...
    # fill area under critical values -----------------------------------------
    if (nxc > 0 && !is.na(xc)) {
      for (i in seq_along(unique(plt_df$func_label))) {
        df_tmp <- subset(plt_df, func_label == unique(plt_df$func_label)[i])
        plt <- plt +
          geom_area(data = df_tmp, mapping = aes(x = x, y = y, fill = flag_xc),
                    alpha = 0.2)
      }
  ...
  plt <- plt +
    ...
    scale_x_continuous(breaks = x_brks, minor_breaks = x_minor_brks,
                       labels = x_labs, limits = c(x_min, x_max),
                       guide = guide_axis(check.overlap = T)) +
    scale_y_continuous(breaks = y_brks, minor_breaks = y_minor_brks,
                       labels = y_labs, limits = c(y_min, y_max),
                       guide = guide_axis(check.overlap = T)) +
    theme_bw() +
    theme(
