...
- Unix style pathname pattern matching, used in bash commands
    ...
    - `pathlib` module in standard library provides [`Path.glob()`](https://docs.python.org/3/library/pathlib.html#pathlib.Path.glob)
...
Regular expressions provide much more advanced features but are slower compared to regular string methods. The python tutorial has a [section](https://docs.python.org/3/howto/regex.html#common-problems) dedicated to describe the issue with choice between using string methods and regular expressions.
